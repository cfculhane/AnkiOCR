...
def on_run_ocr(browser: Browser):
    ...
    assert mw is not None  # keep mypy happy

    selected_nids = list(browser.selected_notes())
    config = mw.addonManager.getConfig(__name__)
    if config is None:
        raise RuntimeError(f"Could not load config name - {__name__}")
    ...
    if config.get("tesseract_install_valid") is not True and config.get("text_output_location") == "new_field":
        showInfo(
            "Note that because this addon changes the note template, you will see a warning about changing the "
            "database and uploading to AnkiWeb. \n "
            "This is normal, and will be shown each time you modify a note template.\n"
            "This message will be only be shown once."
        )
    ...
    try:
        ...
        showInfo(
            f"Processed OCR for {num_notes} notes in {round(time_taken, 1)}s "
            f"({round(time_taken / num_notes, 1)}s per note)\n"
            f"{log_messages}"
        )
        ...
        if progress:
            progress.finish()
        showCritical(
            text="Could not find a valid Tesseract-OCR installation! \n"
            "Please visit the addon page in at https://ankiweb.net/shared/info/450181164 for"
        ...
        if progress:
            progress.finish()
        msg = (
            f"Error encountered during processing. Debug info: \n"
...
def on_menu_setup(browser: Browser):
    ...
    act_run_ocr = QAction(browser, text="Run AnkiOCR on selected notes")  # type: ignore[call-overload]
    act_run_ocr.triggered.connect(lambda b=browser: on_run_ocr(browser))
    anki_ocr_menu.addAction(act_run_ocr)

    act_rm_ocr_fields = QAction(browser, text="Remove OCR data from selected notes")  # type: ignore[call-overload]
